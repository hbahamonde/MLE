...
<<ref, echo = TRUE, warning = FALSE, message = F, fig.align='center', fig.width=6, fig.height=3,dpi=1000,cache=F, include = TRUE>>=
...
p_load(texreg)
texreg(modelo) # usa "screenreg" no "texreg".
@
...
\begin{split}
ln[\frac{\text{Pr}(\text{program: general})}{Pr(\text{program: academic})}]  = \Sexpr{round(coefficients(modelo)[1,1],2)} \Sexpr{round(coefficients(modelo)[1,2],2)}(\text{ses}=\text{middle})  \Sexpr{round(coefficients(modelo)[1,3],2)}(\text{ses}=\text{high}) \Sexpr{round(coefficients(modelo)[1,4],2)}\text{(write)}\\
ln[\frac{\text{Pr}(\text{program: vocation})}{Pr(\text{program: academic})}] = \Sexpr{round(coefficients(modelo)[2,1],2)} + \Sexpr{round(coefficients(modelo)[2,2],2)}(\text{ses}=\text{middle}) \Sexpr{round(coefficients(modelo)[2,3],2)}(\text{ses}=\text{high}) \Sexpr{round(coefficients(modelo)[2,4],2)}\text{(write)}
\end{split}
\end{equation}
...
Ahora interpretaremos el modelo. Tomando la \autoref{inter}, se interpreta como siempre. Por ejemplo, el $\beta_{13}$ significa, cuando \texttt{write} sube una unidad, los {\bf log-odds} de estar en un programa \texttt{general} {\bf versus} un programa \texttt{academic} decrecen \Sexpr{round(coefficients(modelo)[1,4],2)}. Otro ejemplo es el $\beta_{11}$: cuando subo en una unidad \texttt{ses} (es decir de \texttt{low} a \texttt{middle}), los {\bf log-odds} de estar en un programa \texttt{general} {\bf versus} un programa \texttt{academic} decrecen \Sexpr{round(coefficients(modelo)[1,2],2)}.
...
p_load(effects)
plot(effect("ses", modelo))
plot(effect("write", modelo))
@
