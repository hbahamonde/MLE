...
Como ver\'as, cuando $E(y) \;=\; \pi  \;=\; 3$, el {\bf log-likelihood}, es decir, {\bf el valor que maximiza la posibilidad de que la distribucion de $y$ tenga un promedio 3} es $ln \; L(\hat\theta|y)$ = $\Sexpr{round(max(logl1),1)}$ (f\'ijate que reemplazamos $\beta$ por $\hat\theta$ para volver a nuestra notaci\'on MLE). Ve\'amos:
