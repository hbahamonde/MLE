...
y que debido a que $M=\{p,m,u \}$, el modelo m-logit/probit se reduce a $M-1=2$ modelos logit \parencite[163]{Ward2018}. Sustantivamente, lo que esto requiere es que el analista especifique una {\bf categor\'ia de referencia}. Matem\'aticamente, los resultados no se alteran (esto es claro en \autoref{fruta:2}). Pero substantivamente, si importa: cada coeficiente $\hat\boldsymbol{\beta}$ representa el cambio de la categor\'ia base con la categor\'ia que se esta comparando. En consecuencia, importa (desde un punto substantivo cu\'al es tu categor\'ia de referencia).


HACER REPRESENTACION LATENTE
