...
En la distribuci\'on log\'istica los errores est\'an distribuidos de acuerdo a la \emph{distribuci\'on log\'istica est\'andard} con promedio 0 y varianza $\frac{\pi^{2}}{3}\approx3.29$. (Muy lejos de lo que veiamos en la distribuci\'on normal standard, por que{\color{red}?}).
