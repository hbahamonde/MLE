...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% begin knitr stuff
<<setup20, include=FALSE, cache=F, debug=TRUE>>=
...
Por ejemplo, el coeficiente de ``gre'' es $\hat\theta=\Sexpr{logit.1$coefficients["gre"]}$, el $SE = 0.001094$. Entonces, $0.002264426 + 1.96*0.001094$ para el intervalo superior, y $0.002264426 - 1.96*0.001094$ para el intervalo inferior. 
...
Aqu\'i vemos que la variable dependiente (\texttt{admit}) es 55 veces m\'as probable de ocurrir cuando la variable independiente \texttt{gpa} incrementa de su media (\Sexpr{mean(mydata$gpa)}) a 5. Como ves, ganamos interpretatibilidad del modelo. 
...
<<archivo, echo = TRUE, warning = FALSE, message = F, fig.align='center', fig.width=6, fig.height=3,dpi=1000,cache=F, include = TRUE, debug=TRUE>>=
